...
function renderImgs() {
    ...
    var strHtmls = imgs.map(function (img, idx) {
        return `
<div class="img"><a href="#editplace">
<img class="img gallery" onclick="onImageClicked(this.dataset.id)" data-id="${idx + 1} " src="${img.url}"></a>
</div>
`
    })
    var elVendorList = document.querySelector('.gallery');
    elVendorList.innerHTML = strHtmls.join('')
}


function onTypedLine(input) {
    typedLine(input)
    renderMeme()
}
...
function onFColorChange(color) {
    fColorChange(color)
    renderMeme()
    // console.log('font',color)
}

function onSColorChange(color) {
    sColorChange(color)
    renderMeme()
    // console.log('storck',color)
...
function onAddLine() {
    ...
    renderMeme()
    // (console.log('selected is',gMeme.selectedLineIdx))
}

function drawText(text, x, y, size, align, color, strockC, font) {
    gCtx.lineWidth = '1.5'
...
function drawRect(x, y, w) {
    ...
    gCtx.rect(x - size, y - (size * 0.9), w, size) // x,y,width,height
    gCtx.stroke()
